...
from app.models.order_models import DashboardStatsResponse, OrdersListResponse, CheckoutRequest
...
async def checkout(request: CheckoutRequest = None, current_user: dict = Depends(get_current_user_token)):
    """Convert cart items into a confirmed order"""
    try:
        user_id = str(current_user.get("user_id") or current_user.get("id"))
        payment_intent_id = request.payment_intent_id if request else None

        order_id = OrderService.create_order_from_cart(user_id, payment_intent_id)
        if not order_id:
